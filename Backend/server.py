...
ALLOWED_TYPES = ["image/png", "image/jpeg", "image/heic", "image/heif"]
...
async def upload_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        def process_receipt(image_path):
            ...
            text_norm = re.sub(r'[Ss](?=\d)', '5', text_norm)  # S next to digit → 5
            text_norm = re.sub(r'[Ss](?=\d)', '5', text_norm)  # S next to digit → 5
            text_norm = re.sub(r'(?<=\d)[Ss]', '5', text_norm)  # S after digit → 5
            text_norm = re.sub(r'[Zz](?=\d)', '2', text_norm)  # Z next to digit → 2
            text_norm = re.sub(r'(?<=\d)[Zz]', '2', text_norm)  # Z after digit → 2
            text_norm = re.sub(r'[Bb](?=\d)', '8', text_norm)  # B next to digit → 8
            text_norm = re.sub(r'(?<=\d)[Bb]', '8', text_norm)  # B after digit → 8
            text_norm = re.sub(r'[Gg](?=\d)', '9', text_norm)  # G next to digit → 9
            text_norm = re.sub(r'(?<=\d)[Gg]', '9', text_norm)  # G after digit → 9
            ...
            for idx, l in enumerate(line_div_norm):
                prices_in_line = [p for p in price_array if p in l]
                if prices_in_line:
                    if first_price_line is None:
                        first_price_line = idx
                    last_price_line = idx

            filtered_price_array = []
            filtered_line_indices = []

            for idx in range(first_price_line or 0, (last_price_line or 0) + 1):
                l = line_div_norm[idx]
                prices_in_line = [p for p in price_array if p in l]
                if prices_in_line:
                    ...
                    if last_price not in filtered_price_array:
                        ...
                        filtered_line_indices.append(idx)

            price_array = filtered_price_array

            for idx, price in zip(filtered_line_indices, price_array):
                l = line_div[idx]
                norm_l = line_div_norm[idx]

                pos = norm_l.find(price)
                l_split = l[:pos]
                ...
                if "subtotal" in cleaned_name.lower() or "sub total" in cleaned_name.lower(): 
                    continue
            ...
            for s in range(min(len(price_array), len(item_array)) - 1, -1, -1):
                words = item_array[s].strip().split()
                has_real_word = any(len(w) >= 4 and is_real_word(w) for w in words)
        ...
        #         json.dump(itemList, file)
            ...
            return {"id": response.data[0]["id"], "warning": None if items else "No items detected"}
