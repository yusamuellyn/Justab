...
# NEED TO REVAMP THIS AS SOON AS POSSIBLE 
# CURRENTLY ON OLD VERSION FROM GD
@app.post("/upload")
async def upload_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        def process_receipt(image_path):
            ...
            # Preprocessing (Reworked)
           
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # Recolors Image
            gray = cv2.resize(gray, None, fx=1.5, fy=1.5, interpolation=cv2.INTER_CUBIC) # Resizes Image
            _, gray = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU) # Finds Best Image Threshold, Median Blur Removed
